...
kinetica.tables.single_table_per_topic = true
```

`topics` can contain a single topic name or a comma-separated list of topic names. In this test case
it's a single topic getting messages of the same type (Apple).`kinetica.tables.schema_name` is set
to `TEST`. If you provide `kinetica.tables.prefix`, it would be prepended to type name when creating
Kinetica table.
...
in `kinetica.tables.prefix` (results in table outApple), overwriting
table names in `kinetica.tables.destination_name`, and using other config parameters.
...
`kinetica.tables.schema_name` can be provided. If you provide `kinetica.tables.prefix`, it would be
prepended to type name. Do not use param `kinetica.tables.destination_name` as it is ignored for
any configuration with `kinetica.tables.single_table_per_topic = false`.
...
configuration options for Kafka Connector, including supplying Kinetica table with prefix
in `kinetica.tables.prefix` (results in tables outApple, outBanana, outOrange), overwriting
table names in `kinetica.tables.destination_name`, etc.
...
kinetica.tables.create_table = true
kinetica.tables.single_table_per_topic = true
kinetica.schema_evolution.enabled = true
kinetica.schema_evolution.add_new_fields_as_columns = true
kinetica.schema_evolution.make_missing_field_nullable = true
```
